...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=(30, 90), patch_size=1, in_chans=3,
        ...
        self.deep_feature = RDG(G0=embed_dim, C=4, G=24, n_RDB=4)
    ...
    def forward(self, x,y,is_training):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x, xfta = self.deep_feature(x)
            y, yfta = self.deep_feature(y)

            if is_training == 1:
                xT, yT, (M_right_to_left, M_left_to_right), (V_left, V_right) \
                    = self.pam(x, y, xfta, yfta, is_training)
            if is_training == 0:
                xT, yT \
                    = self.pam(x, y, xfta, yfta, is_training)
            ...
            x = self.conv_after_body(self.forward_features(feature_x)) + x
            y = self.conv_after_body(self.forward_features(feature_y)) + y
...
if __name__ == '__main__':
  ...
  # device_ids = [0,1]
    ...
    left = torch.randn((1, 3, 30, 90)).to(device)
    right = torch.randn((1, 3, 30, 90)).to(device)
    SR_left, SR_right, (M_right_to_left, M_left_to_right), (V_left, V_right) = model(left, right, 1)
    print(model)
    ...
    x = model(left, right, is_training = 1)
    end_time = time.time()
